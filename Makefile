...
OBJECT_FILES = crt0.o breakout.o game_logic.o lib.o showpic.o
...
HEADER_FILES = lib.h game_logic.h
...
# Build for standard QEMU 'virt' (RV64, LP64), define STD_VIRT to switch MMIO to UART
compile-for-virt: RISCV_ISA=rv64g
compile-for-virt: RISCV_ABI=lp64
compile-for-virt: CFLAGS+=-DSTD_VIRT
compile-for-virt: LINKER_SCRIPT_FILE=qemu.lds
compile-for-virt: clean $(EXECUTABLE)

# Run on stock QEMU 'virt' with UART on stdio
qemu-virt: compile-for-virt
	$(QEMU_APP) -machine virt -nographic -serial mon:stdio -bios none -kernel $(EXECUTABLE)